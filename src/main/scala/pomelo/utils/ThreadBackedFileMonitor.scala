...
/**
  * A thread based implementation of the FileMonitor. Copied from better.files.ThreadBackedFileMonitor but with a configurable Thread.
  *
  * @param root
  * @param maxDepth
  */
abstract class ThreadBackedFileMonitor(val root: File, maxDepth: Int, threadFactory: ThreadFactory) extends File.Monitor {
  protected[this] val service = root.newWatchService

  private[this] val thread = threadFactory.newThread(() => Iterator.continually(service.take()) foreach process)
  thread.setUncaughtExceptionHandler((thread, exception) => onException(exception))

  def this(root: File, threadFactory: ThreadFactory, recursive: Boolean = true) = this(root, if (recursive) Int.MaxValue else 0, threadFactory)
  ...
  @inline private def repeat(n: Int)(f: => Unit): Unit = (1 to n).foreach(_ => f)
